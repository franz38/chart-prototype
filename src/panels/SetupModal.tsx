...
import { newBarPlot, newLinePlot, newPiePlot, newScatterPlot } from "../state/plots/dto"
...
export const SetupModal = (props: SetupModalProps) => {
    ...
    const generatePlot = (plotType: PlotType) => {
        ...
        switch (plotType) {
            case PlotType.SCATTER: {
                const xAxis = newBottomAxis(props.dataset ? props.dataset.props[0].key : "")
            ...
            case PlotType.PIE: {
                const circularAxis = newCircularAxis(props.dataset ? props.dataset.props.find(p => p.type === "continous")?.key : "")
    ...
    const setupScatter = async () => {
        ...
        const range = (getRange(ds, ds.props[0].key) ?? []).map((_: any) => randomColor())
    ...
    return <>
        <div className={`${step <= 1 ? "" : "hidden"} overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-full max-h-full flex bg-[#00000073]`}>
                        ...
                        <div className="flex flex-col gap-2.5 w-[20rem] mx-auto">
                            {step === 0 && <>
                                ...
                                <div className="flex flex-col gap-2.5">
                                    ...
                                    <button className="text-sm rounded-md" onClick={async () => {
                                        props.setDataset(await loadLocalDs("./world_cups.csv"));
                                        setStep(step + 1)
                                    }}>Random dataset</button>

                                    <button className="text-sm rounded-md" onClick={async () => { setStep(step + 1) }}>No dataset (you can add one later)</button>
                            ...
                            {step === 1 && <div className="flex flex-wrap justify-center mt-2 gap-5 text-sm">
                                <div className="demo-box" onClick={() => { dispatch(toggleMainPanel()); generatePlot(PlotType.SCATTER); setStep(step + 1) }}>
                                    <img src="./plot_illustrations/ScatterPlot.png"></img>
                                    <span>Scatter plot</span>
                                </div>
                                <div className="demo-box" onClick={() => { dispatch(toggleMainPanel()); generatePlot(PlotType.LINE); setStep(step + 1) }}>
                                    <img src="./plot_illustrations/LinePlot.png"></img>
                                    <span>Line plot</span>
                                </div>
                                <div className="demo-box" onClick={() => { dispatch(toggleMainPanel()); generatePlot(PlotType.PIE); setStep(step + 1) }}>
                                    <img src="./plot_illustrations/PiePlot.png"></img>
                                    <span>Pie plot</span>
                                </div>
                                <div className="demo-box" onClick={() => { dispatch(toggleMainPanel()); generatePlot(PlotType.BAR); setStep(step + 1) }}>
                                    <img src="./plot_illustrations/BarPlot.png"></img>
                                    <span>Bar plot</span>
                                </div>
                            </div>}
