...
import { PiePlot, Plot, ScatterPlot, SpiderPlot } from "./state/plots/dto";
...
import { setAces } from "./state/aces/acesSlice";
...
import { changePlotType } from "./state/plots/plotsSlice";
...
export const Canvas = (props: { plotId: string }) => {
    ...
    const onPlotTypeChange = (plot: Plot, newType: PlotType) => {
        ...
        if (newType === PlotType.PIE && plot.type !== PlotType.PIE) {
            dispatch(setAces([newCircularAxis()]))
        }
        else if (newType !== PlotType.PIE && plot.type === PlotType.PIE) {
            dispatch(setAces([newLeftAxis(), newBottomAxis()]))
        }


        dispatch(changePlotType({ plot: plot, newType: newType }))
