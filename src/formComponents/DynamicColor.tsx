...
interface DynamicColorProps {
  ...
  onChange: (
    v: DynamicContinuous<number, string> | DynamicContinuous<string, string>,
  ) => void;
}
...
export const DynamicColor = (props: DynamicColorProps) => {
  const onKeyChange = (val: string) => {
    ...
    } else props.onChange({ ...props.value, key: undefined });
  };
    ...
    <>
      ...
      {!props.value.key && (
        <ColorInput
          ...
          onChange={(val) =>
            props.onChange({ ...props.value, fixedValue: val })
          }
        />
      )}
        ...
        <>
          {typeof props.value.domain[0] === "number" && (
            <ColorGradient
              ...
              onChange={function (c1: string, c2: string): void {
                props.onChange({ ...props.value, range: [c1, c2] });
              }}
            />
          )}
